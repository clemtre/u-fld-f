...
module.exports = function Extract(moduleOptions) {
  ...
  async function process(page) {
    ...
    consola.info(
      `${page.route}: Extractor is replacing ${urls.length} images with local copies`
    )
    return await replaceRemoteImages(page.html, urls).then(
      (html) => (page.html = html)
    )
  }
  ...
  async function replaceRemoteImages(html, urls) {
    await Promise.all(
      urls.map(async (url) => {
        ...
        const name =
          slugify((url.pathname + url.hash).split(ext).join('')) + ext
  ...
  function rewritePayload(payloadPath) {
    ...
    fs.readFile(payloadPath, 'utf8', async (err, data) => {
      ...
      for (const match of matches) {
        ...
        const url = new URL(
          decodeURIComponent(
            JSON.parse('"' + removeTrailingBackslash(match[0]) + '"')
          )
        )
        if (
          baseUrl.hostname === url.hostname &&
  ...
  async function replacePayloadImageLinks(payload, urls) {
    ...
    await Promise.all(
      urls.map((url) => {
        return new Promise((resolve) => {
          ...
          const name = slugify(preName) + ext.split('?')[0]

          let remoteLink = url.href.split('.')
          const end = remoteLink.pop()
          remoteLink =
            encodeSlashes(encodeChars(remoteLink.join('.'))) + '.' + end

          payload = payload
            .split(remoteLink)
            .join(
              encodeSlashes(encodeChars(routerBase + options.path + '/')) + name
            )
          count++
          resolve()
        })
    ...
    consola.info(`Extractor replaced ${count} image links in this payload`)
    return payload
  }
}
function sleep(ms) {
